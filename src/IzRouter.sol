...
interface IzRouter {
    ...
    /// @notice Handle V4 PoolManager swap callback
    /// @param callbackData Encoded callback data
    /// @return result Encoded swap results
    function unlockCallback(bytes calldata callbackData) external payable returns (bytes memory result);
}
